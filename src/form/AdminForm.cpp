...
void showAdminForm(
    ...
    fm.appendToggle("confirm_teleport", "直接传送（开启后选择箱子将直接传送）", false, "§7开启后，点击箱子开关将立即传送，而不是翻页。");
    ...
    if (filteredChests.empty()) {
        ...
        fm.appendHeader("§l§e箱子列表 §r§7(总计: " + std::to_string(filteredChests.size()) + " | 第 " + std::to_string(currentPage) + " / " + std::to_string(totalPages) + " 页)");
        fm.appendDivider();
        ...
        for (int i = startIndex; i < endIndex; ++i) {
            ...
            std::string label = "§b" + ownerName + " §f- " + dimIdToString(chest.dimId) + " §7- " + chestTypeToString(chest.type)
                              + " §r§e[" + std::to_string(chest.pos.x) + ", " + std::to_string(chest.pos.y)
                              + ", " + std::to_string(chest.pos.z) + "]";
                
            std::string toggleName = chest.ownerUuid + "|" + std::to_string(chest.dimId) + "|" + std::to_string(chest.pos.x) + "|" + std::to_string(chest.pos.y) + "|" + std::to_string(chest.pos.z);
    ...
    fm.sendTo(player, [dimIdFilter, chestTypeFilter, coordRange](Player& p, const ll::form::CustomFormResult& result, ll::form::FormCancelReason reason) {
        if (!result.has_value() || result->empty()) {
        ...
        for (const auto& [toggleName, value] : *result) {
             if (toggleName.rfind("dim_", 0) != 0 && toggleName.rfind("type_", 0) != 0 && 
                 toggleName != "confirm_teleport" && toggleName != "page_slider" &&
                 toggleName != "pos1" && toggleName != "pos2") {
        ...
        if (confirmTeleport && !selectedChestToggle.empty()) {
            ...
            if (parts.size() == 5) {
                try {
                    ...
                    p.sendMessage("§a已将你传送至箱子位置: " + std::to_string(x) + ", " + std::to_string(y) + ", " + std::to_string(z));
                    return;

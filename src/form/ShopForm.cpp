...
void showShopChestItemsForm(Player& player, BlockPos pos, int dimId, BlockSource& region) {
        ...
        for (const auto& row : results) {
            ...
            if (!texturePath.empty()) {
                fm.appendButton(
                    ...
                    texturePath,
                    "path",
                    [&fm, &player, pos, dimId, &region, item, itemNbtStr, unitPrice = price](Player& p) {
    ...
    fm.appendButton("返回", [&fm, &player, pos, dimId, &region](Player& p) {
        ...
        bool        isLocked  = true;
        ChestType   chestType = ChestType::Shop;
...
void showShopItemManageForm(
    ...
    fm.appendButton("设置价格", [&fm, &player, item, pos, dimId, &region](Player& p) {
...
void showShopChestManageForm(Player& player, BlockPos pos, int dimId, BlockSource& region) {
    ...
    for (const auto& entry : aggregatedItems) {
        ...
        if (!texturePath.empty()) {
            fm.appendButton(buttonText, texturePath, "path", [&fm, &player, pos, dimId, &region, itemNbtStr](Player& p) {
                showShopItemManageForm(p, itemNbtStr, pos, dimId, region);
            });
        } else {
            fm.appendButton(buttonText, [&fm, &player, pos, dimId, &region, itemNbtStr](Player& p) {
                showShopItemManageForm(p, itemNbtStr, pos, dimId, region);
            });
        }
    }
...
void showShopItemBuyForm(
    ...
    fm.sendTo(
        player,
        [&fm, &player, item, pos, dimId, slot, unitPrice, &region, itemNbtStr](
            Player&                           p,

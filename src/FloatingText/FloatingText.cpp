...
void rebuildFloatingTextForSameDimension(ChestFloatingText& ft) {
    if (ft.debugText) {
        // 历史实现中的 update() 可能把形状同步到错误维度，重建前先对所有在线玩家做一次兜底清理。
        removeFloatingTextFromAllPlayers(ft);
    }
    ft.debugText = createFloatingTextDebugShape(ft);
...
void FloatingTextManager::addOrUpdateFloatingText(
    ...
    if (mFloatingTexts.count(key)) {
        ...
        if (!ft.debugText) {
            rebuildFloatingTextForSameDimension(ft);
            logger
                .debug("已为箱子 ({}, {}, {}) in dim {} 重建悬浮字: {}", ft.pos.x, ft.pos.y, ft.pos.z, dimId, ft.text);
        }
    } else {
        // 创建新的悬浮字（使用常量定义的偏移量）
        mFloatingTexts.emplace(key, ChestFloatingText(pos, dimId, ownerUuid, text, type));
        rebuildFloatingTextForSameDimension(mFloatingTexts.at(key));
...
void FloatingTextManager::removeFloatingText(BlockPos pos, int dimId) {
    ...
    if (mFloatingTexts.count(key)) {
        ...
        if (!ft.playerFakeItemStates.empty()) {
            ...
            if (level) {
                level->forEachPlayer([&ft](Player& player) {
                    auto it = ft.playerFakeItemStates.find(player.getUuid().asString());
                    if (it != ft.playerFakeItemStates.end()) {
                        RemoveFakeitem(player, it->second.actorId);
                        ft.playerFakeItemStates.erase(it);
                    }
                    return true;
                });
            }
            ft.playerFakeItemStates.clear();
        }
...
void FloatingTextManager::setFloatingTextVisible(BlockPos pos, int dimId, bool visible) {
    std::unique_lock<std::shared_mutex> lock(mFloatingTextsMutex);
    auto                                key = std::make_pair(dimId, pos);
    auto                                it  = mFloatingTexts.find(key);
    if (it == mFloatingTexts.end()) return;
...
void FloatingTextManager::setChestFakeItemEnabled(BlockPos pos, int dimId, bool enable) {
    bool shouldUpdateFakeItems = false;
    {
        ...
        auto& ft          = it->second;
        ft.enableFakeItem = enable;

        if (!enable) {
            if (!ft.playerFakeItemStates.empty()) {
                auto level = ll::service::getLevel();
                if (level) {
                    for (const auto& [playerUuid, fakeItemState] : ft.playerFakeItemStates) {
                        auto* playerPtr = findOnlinePlayerByUuidString(*level, playerUuid);
                        if (playerPtr) {
                            RemoveFakeitem(*playerPtr, fakeItemState.actorId);
                        }
                    }
                }
            }
            // 保留映射，交由后续更新循环继续重试清理，避免单次发包失败后残留。
            shouldUpdateFakeItems = true;
        } else if (ft.isDynamic && !ft.itemNbts.empty()) {
            shouldUpdateFakeItems = true;
        }
    }
...
void FloatingTextManager::drawAllFloatingTexts(Player& player) {
    drawAllFloatingTexts(player, player.getDimensionId());
}
 ...
 *
 * @details 更新策略：
 * - 悬浮字与假物品同步切换（同一轮更新同一个物品索引）
 * - 维护性任务（库存同步/动态价格重置）隔轮执行，降低开销
...
ll::coro::CoroTask<> FloatingTextManager::dynamicTextUpdateCoroutine() {
    ...
    while (!mShouldStopUpdate.load()) {
        ...
        // === 第三阶段：批量写回更新（写锁，快速操作） ===
        {
            ...
            for (const auto& info : toUpdate) {
                ...
                if (updateText && !ft.itemNames.empty()) {
                    ...
                    if (!ft.itemNbts.empty()) {
                        // 假物品与悬浮字使用同一轮播索引，确保同帧切换。
                        ft.currentFakeItemIndex = ft.currentItemIndex % ft.itemNbts.size();
                    }
                    ft.text =
        ...
        bool shouldRefreshFakeItems = updateText && !toUpdate.empty();

        if (!updateText) {
            // 与箱子真实库存同步，处理“手动取出物品”导致的展示残留。
            std::vector<std::tuple<BlockPos, int, ChestType>> shopsToRefresh;
            {
                std::shared_lock<std::shared_mutex> lock(mFloatingTextsMutex);
                shopsToRefresh.reserve(mFloatingTexts.size());
                for (const auto& [key, ft] : mFloatingTexts) {
                    if (ft.isDynamic && (ft.type == ChestType::Shop || ft.type == ChestType::AdminShop)) {
                        shopsToRefresh.emplace_back(ft.pos, ft.dimId, ft.type);
                    }
                }
            }
            for (const auto& [shopPos, shopDimId, shopType] : shopsToRefresh) {
                shouldRefreshFakeItems =
                    updateShopFloatingText(shopPos, shopDimId, shopType, false) || shouldRefreshFakeItems;
            }

            // 检查并重置过期的动态价格计数器
            DynamicPricingService::getInstance().checkAndResetCounters();
        }
        if (shouldRefreshFakeItems) {
            updateFakeItemsForAllPlayers();
        }
...
// 更新商店/回收商店的悬浮字物品列表
bool FloatingTextManager::updateShopFloatingText(BlockPos pos, int dimId, ChestType type, bool refreshFakeItems) {
...

    {
        ...
        if (isShopType) {
            itemResults = db.query(
                "SELECT si.item_id, id.item_nbt, si.db_count "
                "FROM shop_items si JOIN item_definitions id ON si.item_id = id.item_id WHERE "
                ...
                if (region) {
                    ...
                    if (type == ChestType::Shop && realStock && *realStock != dbStock) {
                        if (ShopRepository::getInstance().updateDbCount(pos, dimId, itemId, *realStock)) {
                            ...
                            logger.warn(
                                "updateShopFloatingText: 库存回写失败 ({}, {}, {}) dim {} item {}",
                                pos.x,
                                pos.y,
                                pos.z,
                                dimId,
    ...
    bool changed               = false;
    {
        ...
        if (!ft.itemNames.empty()) {
            ...
            newText = TextService::getInstance().generateDynamicShopText(type, ft.itemNames[ft.currentItemIndex]);
        } else {
            ft.currentItemIndex = 0;
            newText             = TextService::getInstance().generateEmptyShopText(type);
        }
        ...
        shouldUpdateFakeItems = refreshFakeItems && changed;
    } // 写锁在此释放
...
void FloatingTextManager::sendFakeItemToPlayer(Player& player, ChestFloatingText& ft) {
    ...
    if (!shouldDisplayFakeItemToPlayer(player, ft)) {
...
void FloatingTextManager::removeFakeItemFromPlayer(Player& player, ChestFloatingText& ft) {
    std::string playerUuid = player.getUuid().asString();
    auto        it         = ft.playerFakeItemStates.find(playerUuid);
    if (it != ft.playerFakeItemStates.end()) {
        RemoveFakeitem(player, it->second.actorId);
        ft.playerFakeItemStates.erase(it);
    }
...
void FloatingTextManager::updateFakeItemsForAllPlayers() {
    ...
    if (!CT::ConfigManager::getInstance().get().floatingText.enableFakeItem) {
        // 全局关闭后主动清理所有在线玩家已显示的假物品，避免残留。
        std::unique_lock<std::shared_mutex> lock(mFloatingTextsMutex);
        level->forEachPlayer([this](Player& player) {
            std::string playerUuid = player.getUuid().asString();
            for (auto& [_, ft] : mFloatingTexts) {
                auto it = ft.playerFakeItemStates.find(playerUuid);
                if (it != ft.playerFakeItemStates.end()) {
                    RemoveFakeitem(player, it->second.actorId);
                    ft.playerFakeItemStates.erase(it);
    ...
    level->forEachPlayer([this, &fakeItemsByDim](Player& player) {
        ...
        for (const auto& key : dimIt->second) {
            ...
            auto& ft = it->second;
            // 再次检查条件（可能在锁释放期间被修改）
            if (ft.isDynamic && ft.dimId == playerDimId) {
                if (!ft.enableFakeItem || ft.itemNbts.empty() || !shouldDisplayFakeItemToPlayer(player, ft)) {
                    removeFakeItemFromPlayer(player, ft);
                } else {
 ...
 *
 * 当玩家离线时调用此方法，从所有悬浮字的 playerFakeItemStates 中移除该玩家的记录。
 * 这可以防止长期运行时的内存泄漏。
 *
...
void FloatingTextManager::cleanupOfflinePlayerFakeItems() {
    ...
    for (auto& [key, ft] : mFloatingTexts) {
        for (auto it = ft.playerFakeItemStates.begin(); it != ft.playerFakeItemStates.end();) {
            if (onlinePlayerUuids.find(it->first) == onlinePlayerUuids.end()) {
                it = ft.playerFakeItemStates.erase(it); // 离线玩家，清理
                totalRemoved++;
            } else {
                ++it;
            }
